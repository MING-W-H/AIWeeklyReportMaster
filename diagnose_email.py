...
def main():
    if not CONFIG_PATH.exists():
        logger.error("找不到配置文件: %s", CONFIG_PATH)
        return 1
    ...
    logger.info("=" * 60)
    logger.info("腾讯企业邮箱 SMTP 诊断")
    logger.info("=" * 60)
    logger.info("发件人    : %s", sender)
    # 安全：仅打印密码长度和脱敏占位，不输出密码本身或 repr（避免日志/截图泄露）
    logger.info("密码长度  : %d 字符", len(password))
    logger.info("密码预览  : %s（已脱敏）", "*" * min(len(password), 4))
    logger.info("SMTP 主机 : %s", smtp_host)
    logger.info("SMTP 端口 : %s", smtp_port)
    logger.info("=" * 60)

    # 1. 检查必填字段
    if not sender:
        logger.error("[FAIL] email.sender 为空")
        return 1
    if not password:
        logger.error("[FAIL] email.password 为空")
        return 1

    # 2. 检查发件人邮箱格式
    if "@" not in sender or "." not in sender.split("@")[-1]:
        logger.error("[FAIL] 发件人邮箱格式异常: %s", sender)
        return 1
    logger.info("[OK] 发件人邮箱格式正常")

    # 3. 检查密码长度（腾讯企业邮箱客户端专用密码通常为 16 位）
    if len(password) != 16:
        logger.warning("[WARN] 密码长度 %d 位，腾讯企业邮箱客户端专用密码通常为 16 位", len(password))
        logger.warning("       请确认你生成的是「客户端专用密码」而非「登录密码」")
    else:
        logger.info("[OK] 密码长度 16 位，符合客户端专用密码格式")

    # 4. 检查密码是否包含空格或特殊字符
    if " " in password or "\n" in password or "\r" in password:
        logger.error("[FAIL] 密码中包含空格或换行符，请重新复制")
        return 1
    logger.info("[OK] 密码无空格/换行符")
    ...
    try:
        ...
        # 尝试发送一封测试邮件给自己
        logger.info("")
        logger.info("[..] 尝试发送测试邮件给 %s ...", sender)
        msg = MIMEText("这是一封来自 AI 周报系统的 SMTP 诊断测试邮件。", "plain", "utf-8")
        msg["From"] = sender
        msg["To"] = sender
        msg["Subject"] = "AI 周报系统 - 邮件测试"
        server.sendmail(sender, [sender], msg.as_string())
        logger.info("[OK] 测试邮件发送成功！请检查 %s 收件箱", sender)
        server.quit()
        logger.info("")
        logger.info("=" * 60)
        logger.info("诊断结论：邮箱配置完全正常，可正常运行 weekly_report.py")
        logger.info("=" * 60)
        return 0
    except smtplib.SMTPAuthenticationError as e:
        logger.error("[FAIL] 登录失败 (SMTPAuthenticationError)")
        logger.error("       错误码: %s", e.smtp_code)
        logger.error("       错误信息: %s", e.smtp_error.decode("utf-8", errors="replace") if isinstance(e.smtp_error, bytes) else e.smtp_error)
    except smtplib.SMTPConnectError as e:
        logger.error("[FAIL] 连接失败 (SMTPConnectError): %s", e)
    except smtplib.SMTPException as e:
        logger.error("[FAIL] SMTP 异常: %s", e)
    except Exception as e:
        logger.error("[FAIL] 未知异常: %s: %s", type(e).__name__, e)
    ...
    try:
        ...
        logger.error("[FAIL] 587 连接失败: %s: %s", type(e).__name__, e)

    # 7. 诊断结论
    logger.info("")
    logger.info("=" * 60)
    logger.info("诊断结论：鉴权失败，可能原因如下：")
    logger.info("=" * 60)
    logger.info("""
