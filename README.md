...
4. 程序通过 Stream 长连接实时接收回复：
   ...
   - 「取消」→ 机器人询问审核人认为存在哪些问题，并等待其反馈
5. 收到问题反馈后，程序将反馈重新发送给大模型，生成修订版周报并再次推送审核：
   - 该「反馈 → 重新生成 → 再审核」可多轮循环，直至审核人回复「发送」确认
   - 反馈阶段回复「放弃」等取消关键词 → 放弃发送（周报文件仍保留在 `reports/`）
   - 任一环节超时未回复（默认 30 分钟）→ 自动放弃并通知审核人
