...
FORMAT_TEMPLATES: Dict[str, str] = {
    "markdown": (
        "请将以下 Excel 汇总数据整理为 Markdown 格式的周报，要求精简、聚焦进度，按以下结构输出：\n\n"
        "# 周报\n\n"
        "## 一、本周涉及项目\n"
        "（一句话列出本周涉及的主要项目名称即可）\n\n"
        "## 二、各项目情况\n"
        "（每个项目一节，只写四件事，用简短要点表达，不展开细节、不罗列具体工作项）\n\n"
        "### 项目名称\n"
        "- 已完成：本周完成了 xxx（一句话）\n"
        "- 未完成：xxx\n"
        "- 还需时间：约 X 天 / X 周\n"
        "- 预计完成：X 月 X 日前\n\n"
        "要求：\n"
        "1. 已完成、未完成各用一句概况，不要逐条罗列具体任务\n"
        "2. 「还需时间」「预计完成」必须综合工作描述与实际工时合理估算，"
        "用「预计/约」等措辞，严禁出现「待确认」或留空\n"
        "3. 严格控制篇幅，单个项目不超过 5 行，杜绝套话与重复\n"
        "4. 语言简练、重点突出、数据准确"
    ),
    "plain": (
